...
public class Giraffe extends NaturalistAnimal implements NaturalistGeoEntity {
    ...
    protected void positionRider(Entity passenger, MoveFunction callback) {
        ...
        callback.accept(passenger, this.getX(), this.getY() + this.getBbHeight() * 0.6, this.getZ());
    ...
    private PlayState predicate(final AnimationTest<Giraffe> event) {
        if (this.isBaby()) {
            event.setControllerSpeed((float)(0.56F * Math.max(0.1, this.walkAnimation.speed() * 3.0)));
        } else {
